...
namespace Assets.Grass
{
    class RandomTuftGenerator
    {
        ...
        {
            return basePlantBendingStiffness.Lerp(Random.value);
        }

        public static float GetPlantBendingValue(MyRange basePlantBendingValue)
        {
            return basePlantBendingValue.Lerp(Random.value);
        }

        public static int GetTuftCount(MyRange elementsRange)
        {
            return (int)Random.Range(elementsRange.Min, elementsRange.Max);
        }
        ...
        {
            var min = 3;
            return new MyRange(min, min + Random.Range(0, 8));
        }
